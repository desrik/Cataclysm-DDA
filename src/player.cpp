...
void player::practice( Skill *s, int amount, int cap )
{
    ...
    if (skillLevel(s) > cap && !(s->ident() == "str")) { //blunt grinding cap implementation for crafting
        amount = 0;
    ...
    if (amount > 0 && level.isTraining()) {
        ...
        if (is_player() && newLevel > oldLevel) {
          if( s->ident() == "str" || s->ident() == "dex" || s->ident() == "int" || s->ident() == "per" ) {
            if( s->ident() == "str") {
              per_max = newLevel;
            } else if( s->ident() == "dex") {
              per_max = newLevel;
            ...
            add_msg(m_good, _("Your %s has increased to %d!"), s->name().c_str(), newLevel);
          } else {
            add_msg(m_good, _("Your skill in %s has increased to %d!"), s->name().c_str(), newLevel);
          }
        }
        if(is_player() && newLevel > cap && !( s->ident() == "str" || s->ident() == "dex" || s->ident() == "int" || s->ident() == "per" )) {
